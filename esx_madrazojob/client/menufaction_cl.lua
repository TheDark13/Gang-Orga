          ...
          function(data2, menu2)
            ...
            if distance ~= -1 and distance <= 3.0 then
              ...
              if data2.current.value == 'handcuff' then
                TriggerServerEvent('esx_madrazo:handcuff', GetPlayerServerId(player))
              end

              if data2.current.value == 'drag' then
                TriggerServerEvent('esx_madrazo:drag', GetPlayerServerId(player))
              end

              if data2.current.value == 'put_in_vehicle' then
                TriggerServerEvent('esx_madrazo:putInVehicle', GetPlayerServerId(player))
              end

              if data2.current.value == 'out_the_vehicle' then
                TriggerServerEvent('esx_madrazo:OutVehicle', GetPlayerServerId(player))
              end
...
function OpenBodySearchMenu(player)
  ...
  ESX.TriggerServerCallback('esx_madrazo:getOtherPlayerData', function(data)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value ~= nil then
          ...
          TriggerServerEvent('esx_madrazo:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
function OpenVehicleInfosMenu(vehicleData)
  ...
  ESX.TriggerServerCallback('esx_madrazo:getVehicleInfos', function(infos)
...
RegisterNetEvent('esx_madrazo:handcuff')
AddEventHandler('esx_madrazo:handcuff', function()
...
RegisterNetEvent('esx_madrazo:drag')
AddEventHandler('esx_madrazo:drag', function(cop)
...
RegisterNetEvent('esx_madrazo:putInVehicle')
AddEventHandler('esx_madrazo:putInVehicle', function()
...
RegisterNetEvent('esx_madrazo:OutVehicle')
AddEventHandler('esx_madrazo:OutVehicle', function(t)
...
Citizen.CreateThread(function()
    while true do
		...
		if IsControlPressed(0,  Keys['F9']) and PlayerData.job2 ~= nil and PlayerData.job2.name == 'madrazo' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'bandits_actions') and (GetGameTimer() - GUI.Time) > 150 then
		   OpenBanditsMenu()
        end 
    end
end)
